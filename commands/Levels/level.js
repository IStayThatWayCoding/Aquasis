...
module.exports = {
    ...
    run: async (bot, message, args) => {
        ...
        const background = await Canvas.loadImage('./rankbg.png');
        ...
        await mongo().then(async mongoose => {
            try {
                ...
                if (results.length === 0) {
                    return message.channel.send(`This user isn't ranked yet! (send messages, or try running the command again)`)
                ...
                for (const info of results) {
                    ...
                    ctx.fillText(`Level ${level}`, canvas.width / 3.8, canvas.height / 1.6);
